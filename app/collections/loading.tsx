      ...
      <header className="border-b border-stone-200 bg-white">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4">
          <div className="flex items-center gap-3">
            <LogoMark />
            <div>
              <h1 className="font-serif text-2xl font-bold">集字作品</h1>
              <p className="text-sm text-stone-500">讀取儲存的集字內容</p>
            </div>
          </div>
      ...
      <section className="mx-auto max-w-6xl px-4 py-8">
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          {[0, 1, 2, 3, 4, 5].map((item) => (
            <div key={item} className="rounded-2xl border border-stone-200 bg-white p-4">
              ...
              <div className="mb-4 grid min-h-[88px] grid-cols-4 gap-2 rounded-2xl bg-stone-50 p-3">
                {[0, 1, 2, 3].map((glyph) => (
                  <div key={glyph} className="h-16 animate-pulse rounded-xl bg-white" />
                ))}
              </div>
